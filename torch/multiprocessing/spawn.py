...
def _wrap(fn, i, args, error_file):
    ...
    try:
        ...
        with open(error_file, "w+b") as fh:
...
class ProcessContext:
    ...
    def join(self, timeout=None):
        ...
        with open(self.error_files[error_index], "r+b") as fh:
...
def start_processes(
    ...
    for i in range(nprocs):
        ...
        os.unlink(tf.name)  # Delete the error file so we can just check existence above
        process = mp.Process(
            target=_wrap,
